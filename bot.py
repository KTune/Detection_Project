...
def handle_face_swap(message):
    ...
    #     bot.send_message(message.chat.id, e)
@bot.message_handler(content_types=['photo'])
def handle_large_eyes(message):
    try:
        file_info = bot.get_file(message.photo[len(message.photo) - 1].file_id)
        downloaded_file = bot.download_file(file_info.file_path)

        with open("photos/eyes" + str(message.message_id) + ".jpg", 'wb') as new_file:
            new_file.write(downloaded_file)
        big_eyes("photos/eyes" + str(message.message_id) + ".jpg", message)
        result = "photos/eyes" + str(message.message_id) + ".jpg"
        bot.send_photo(message.chat.id, (open(result, "rb")))
        os.remove("photos/eyes" + str(message.message_id) + ".jpg")
        os.remove(result)
    except Exception as e:
        bot.send_message(message.chat.id, e)
